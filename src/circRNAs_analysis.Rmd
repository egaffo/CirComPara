---
title: "Circpipe: detection and analysis of circular RNAs"
date: "`r Sys.Date()`"  
output: 
  html_document: 
    keep_md: no
...
# Circpipe results summary
Circpipe analysis results are presented in this page. Different sections display aggregated data in tables and figures. Raw and integrated results were saved in comma separated files (CSV). 
...
if(nrow(circrnas.gtf) > 0){
  ...
  #   circrnas.gtf["segecirc", V4 := V4-1]
  #   circrnas.gtf["segecirc", `:=`(gene_id = paste0(V1, ":", V4, "-", V5, ":", V7), 
...
kable(data.frame(sample = names(sample.table.file.names), file = sample.table.file.names, row.names = NULL))#, 
#      caption = "Files reporting the detected circRNAs in each sample")
```

Backsplices were detected using `r circrnas.gtf[, .N, by = V2][, .N]` programs in parallel: `r paste0(circrnas.gtf[, .N, by = V2][, V2], collapse = ", ")`
...
with at least 2 reads | with at least 2 reads and 2 methods
---------------: | -----------------------------------------:
...
kable(circ.per.method[, .N, by = V2][, Method := V2][order(-N), .(Method, N)])
...
circByMethod.barplot <- ggplot(data = circ.per.method, mapping = aes(x = V2, fill = V2)) + 
  ...
  ggtitle("CircRNAs detected with >= 2 reads by each method")
...
sharedByMethod.barplot <- ggplot(data = circ.per.method[, .(shared_by = .N), by = gene_id], 
  ...
  ggtitle("Number of circRNAs (with >= 2 reads)\ncommonly detected by the methods")
## compute limits to remove space from x-axis. Add a 10% increase to the top limit
...
circ.2reads.at.least <- circrnas.gtf[RPM > 0 , .N, by = .(sample_id, gene_id)][, .("With at least 2 reads" = .N), by = sample_id]
circ.2reads.at.least.2methods.at.least <- circrnas.morethan2methods.medrpm[, .N, 
                                                                           by = .(sample_id, circ_id)][
                                                                             , .("With at least 2 reads and 2 methods" = .N), 
                                                                             by = sample_id]
...
Number of genes overlapping the expressed circRNAs: `r circRNAs.per.gene[, .N]`
...
Considering circRNAs detected by at least one method, the table below indicate the mumber of circRNAs falling in each annotation class according to backsplice start and end positions in relation to annotated exon or introns from overlapping genes.   
...
kable(circrna.rpms.genes[, .N, by = .(circ_id, label)][, .N, by = label][order(-N)], 
      caption = "")
```
...
corr.density.plot <- corr.density.plot + theme_bw() + ggtitle("CircRNA-gene expression correlation distribution") + 
  xlab("Spearman correlation") + ylab("") + 
