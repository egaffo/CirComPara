...
meta.file.out <- "sample_conditions.csv"
fwrite(x = meta, file = meta.file.out, row.names = F)
```

Samples' metadata have been saved in file <a href="`r meta.file.out`">`r meta.file.out`</a>.  
...
merged.circ.to.genes.file <- "circ_to_genes.csv"
fwrite(circ_to_genes, merged.circ.to.genes.file, row.names = F)
```

CircRNA host gene annotation (one row per circRNA) was saved in <a href="`r merged.circ.to.genes.file`">`r merged.circ.to.genes.file`</a>.  
...
```{r}
## save merged host gene annotation
merged.circ.to.gene.file <- "gene_to_circ.csv"
fwrite(circ_to_gene, merged.circ.to.gene.file, row.names = F)
```

Genes expressing circRNAs are listed in <a href="`r merged.circ.to.gene.file`">`r merged.circ.to.gene.file`</a>.  
