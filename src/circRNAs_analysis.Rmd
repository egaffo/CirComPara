...
Select only circRNAs detected with >= `r min_reads` reads by at least one method. Keep also circRNAs detected with < `r min_reads` reads if they are detected with >= `r min_reads` reads by any method in any sample.  
...
## (not normalized values) by at least one method. Keep also the circrnas 
## detected by < 2 reads if another method detected them with >= 2 reads.

detected_circ_ids <- circrnas.gtf[reads >= min_reads, unique(circ_id)]
circ.per.method <- circrnas.gtf[circ_id %in% detected_circ_ids, 
                                .N, by = .(circ_id, method)]
...
circ_met <- 
    merge(circ_n_met[, .(circ_id, reliable.circrna, overall.n.methods)],
          circ_met[, .(circ_id, reliable.circrna, overall.circ.methods)], 
...
---------------: | -----------------------------------------:
`r circrnas.gtf[reads >= min_reads, .N, by = .(circ_id)][, .N]` | `r circrnas.morethan2methods.medrpm.table[, .N]`
...
circ.2reads.at.least <- 
    circrnas.gtf[reads >= min_reads , .N, 
...
circrnas.morethan2methods.medrpm <- 
    melt(circrna.rpms[reliable.circrna == 1], 
         id.vars = "circ_id", 
...
`r n.circ.per.gene[, .N]` genes overlap the `r circrnas.morethan2methods.medrpm.table[, .N]` reliable circRNAs ( _intergenic_ circRNAs were not considered)  

Number of circRNAs expressed by each gene, with the respective circRNAs IDs, are reported in file <a href="`r basename(circRNAs_per_gene.file)`">`r basename(circRNAs_per_gene.file)`</a>.  
...
circRNAs.to.gene.per.sample <- 
    merge(circrnas.morethan2methods.medrpm[, .(circ_id, sample_id)], 
          ...
          by = "circ_id", all.x = T, all.y = F, allow.cartesian = T)
