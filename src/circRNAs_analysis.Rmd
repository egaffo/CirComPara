...
if(nrow(circrnas.gtf[V6 >= min_reads]) > 0){
  ...
  circrna.median.reads <- circrnas.gtf[, .(median.reads = median(as.numeric(V6)),
                                           n_methods = .N), 
                               by = .(sample_id, gene_id)]
  
  ## >= min_reads circrnas & >= min_methods circrnas
  ...
  ## make wide expression table circ ~ method
  circrna.rpms <- 
      dcast(data = circrnas.gtf, fun.aggregate = sum,
            formula = sample_id + gene_id + V1 + V4 + V5 ~ V2, 
            value.var = "V6", fill = 0)
  ...
  circrnas.gtf[, `:=`(median.reads="", gene_id="")]
  circrna.rpms <- circrnas.gtf
  circrna.rpms.genes <- circrna.rpms
  circrna.rpms.genes[, `:=`(circ_id="", gene_ids="", gene_names="", label="", 
                            sample_id="", chr="", V1="", start="", V4="", 
                            end="", V5="", strand="", median.reads="")]
}
...
Backsplices were detected by `r circrnas.gtf[, .N, by = V2][, .N]` different methods: `r paste0(sort(circrnas.gtf[, .N, by = V2][, V2]), collapse = ", ")`
...
circrnas.morethan2methods <- circrnas.gtf[gene_id %in% meth.filtered.circ.ids]
...
setkey(circrna.rpms.genes, circ_id, sample_id)
setkey(circrnas.morethan2methods, gene_id, sample_id)

circrnas.morethan2methods.medrpm <- 
    unique(circrna.rpms.genes[circrnas.morethan2methods][median.reads > 0, 
                                                  .(circ_id, gene_ids, gene_names, 
                                                    label = simple_gene_region, 
                                                    sample_id, chr = V1, 
                                                    start = V4, end = V5, 
                                                    strand, median.reads)])
...
if(nrow(circrnas.morethan2methods) > 0 ){
  ...
  circrnas.morethan2methods.medrpm.table <- 
      dcast(data = circrnas.morethan2methods.medrpm, 
            formula = circ_id + gene_ids + gene_names + label + 
                chr + start + end + strand ~ sample_id,
            value.var = "median.reads", fill = 0, fun.aggregate = median)
}
## save table
...
kshow <- data.frame(shared.counts.table)
colnames(kshow) <- sub("_", " ", colnames(kshow))
rownames(kshow) <- sub("_", " ", rownames(kshow))
datatable(kshow, caption = "")
```
...
sharedByMethod.barplot <- ggplot(data = circ.per.method[, .(shared_by = .N), 
                                                        by = gene_id], 
...
## compute limits to remove space from x-axis. Add a 10% increase to the top limit
sharedByMethod.barplot.limits <- 
    c(0, max(circ.per.method[, .(shared_by = .N), 
                             by = gene_id][, .N, by=shared_by][, N])*1.1)
...
---------------: | -----------------------------------------:
`r circrnas.gtf[V6 >= min_reads, .N, by = .(gene_id)][, .N]` | `r circrnas.morethan2methods.medrpm.table[, .N]`
...
```{r}
# The fields of the table header are explained below:
# |Field     |Description                                                                        |
# |----------|---------------------------------------------------------------------------------- |
...
# \*\* CircRNA expression is reported in median raw count of circRNA detection methods.  
```

## CircRNAs detected by sample

```{r show_circrnas_2reads_2methods_sample, echo=FALSE, include=TRUE}
circ.2reads.at.least <- circrnas.gtf[V6 > 0 , .N, 
...
circ.cat.cond.xpr <- merge(circrnas.morethan2methods.medrpm[
    , .(circ_id, median.reads, sample_id, sorted_label)], 
    intgroup.dt[, .(condition, sample_id)], by = "sample_id")
...
# circrna.cat.xpr.boxplot <- 
#     ggplot(data = circ.cat.cond.xpr, 
#            mapping = aes(x = simple_label, y = median.reads, 
#                          fill = simple_label)) + 
...
# 
# ## group by condition
# circrna.cat.xpr.per.cond.boxplot <- 
#     ggplot(data = circ.cat.cond.xpr, 
#            mapping = aes(x = condition, y = median.reads, fill = simple_label)) + 
#     geom_boxplot(notch = F) + 
#     scale_y_log10() + 
#     xlab(NULL) + 
#     ylab("Reads") + 
...
#     ggplot(data = circ.cat.cond.xpr, 
#            mapping = aes(x = sample_id, y = median.reads, fill = simple_label)) + 
#     geom_boxplot(notch = F) + 
#     scale_y_log10() + 
#     facet_grid(facets = . ~ condition, drop = T, scales = "free_x") + 
#     xlab(NULL) + 
#     ylab("Reads") + 
#     scale_fill_discrete("Category") +
...
_intergenic_ circRNAs were not considered.  
...
circRNAs.to.gene <- subset(circ_to_gene[gene_region != "intergenic"], 
...
## compute limits to remove space from x-axis. Add a 10% increase to the top limit
circRNAs.per.gene.plot.limits <- 
    c(0, max(circRNAs.per.gene[, .N, by=circRNAs_per_gene][, N])*1.1)
...
circRNAs.to.gene.per.sample <- 
    merge(circrnas.morethan2methods.medrpm[, .(circ_id, sample_id)], 
          circRNAs.to.gene[, .(circ_id, gene_id, gene_name)], 
          by = "circ_id")

n.circ.per.gene.per.sample <- 
    circRNAs.to.gene.per.sample[, .(n_circRNAs = .N, 
...
```{r}
fig.height <- max(ceiling(meta[, .N]/3), 2)*1.5
```
...
if(intgroup.dt[, .N, by=condition][, .N] > 1){
    ...
    # kable(sapply(X = split(circRNAs.per.gene.per.condition,
    #                        circRNAs.per.gene.per.condition$condition), 
...
#     ggtitle("CircRNAs per gene vs circRNA expression") + 
#     xlab("Number of circRNAs") + 
#     ylab("CircRNA reads") + 
#     theme_bw() + 
...
# circ.expr.to.circ.per.gene <- 
#     unique(circ.expr.to.circ.per.gene.per.sample[, .(avgReads = mean(median.reads)), 
#                                                  by = .(gene_id, 
#                                                         circ_id)])[, circRNAs_per_gene := .N, 
#                                                                    by = gene_id][]
...
#     circ.expr.to.circ.per.gene.per.condition <- 
#         unique(circ.expr.to.circ.per.gene.per.sample[, .(avgReads = mean(median.reads)), 
#                                                      by = .(condition, gene_id,
#                                                             circ_id)])[, circRNAs_per_gene := .N, 
#                                                                        by = .(condition, gene_id)][]
#     circ.expr.to.circ.per.gene.per.condition.plot <- 
#         ggplot(data = circ.expr.to.circ.per.gene.per.condition, 
#                mapping = aes(x = factor(circRNAs_per_gene), 
#                              y = avgReads,
#                              fill = condition)) + 
#         geom_boxplot(varwidth = T) + scale_y_log10() +  
...
#         ggtitle("CircRNAs per gene vs circRNA expression") + xlab("Number of circRNAs") + ylab("CircRNA reads") + 
#         theme_bw() + 
#         theme(text = element_text(size=default.font.size), axis.text.x = element_text(angle=90, hjust=1))
#     
#     circ.expr.to.circ.per.gene.per.condition.plot <- circ.expr.to.circ.per.gene.per.condition.plot + 
...
```{r}
## normalize linear counts
# dds.lin <- 
#     DESeqDataSetFromMatrix(as.matrix(bks_linear_counts.tab.gz, 
#                                      rownames = "circ_id")[, as.character(meta$sample_id)],
#                            colData = data.frame(meta, row.names = "sample_id"), 
#                            design = ~1)
# dds.lin <- estimateSizeFactors(dds.lin, type = "poscounts")
# lin.norm.counts <- counts(dds.lin, normalized = T)
# lin.norm.counts.m <- as.data.table(melt(lin.norm.counts, 
#                           varnames = c("gene_id", "sample_id"), 
#                           value.name = "lin.reads"))

## compute CLR
# circ.lin.xpr <- merge(circrna.rpms[circrnas.morethan2methods],
#       lin.norm.counts.m, 
#       by = c("sample_id", 
#              "gene_id"),
#       all.x = T, all.y = F)[, .(sample_id, circ_id = gene_id, 
#                                 circ.reads = median.reads, 
#                                 lin.reads, CLR = (2*median.reads)/lin.reads)]
circ.lin.xpr <- 
    merge(circrna.rpms[gene_id %in% meth.filtered.circ.ids, 
                       .(sample_id, circ_id = gene_id, median.reads)],
          bks_linear_counts.tab.gz.m[circ_id %in% meth.filtered.circ.ids], 
...
# ggplot(data = circ.lin.xpr, mapping = aes(x = circ.reads, 
#                                           y = lin.reads)) +
#     geom_point(aes(color = CLR)) +
#     # geom_smooth(method = "lm", se = F) +
#     scale_x_log10("Circular norm reads", label = comma) +
#     scale_y_log10("Linear norm reads", label = comma) +
#     # coord_fixed(ratio = 1) +
...
#     scale_y_log10(name = "Linear CPM", label = comma) +
#     scale_fill_manual(values = setNames(intgroup.dt[, color], 
#                                         nm = intgroup.dt[, sample_id]), 
#                       guide = F) +
...
```{r}
# plot.dt <- circ.lin.cor[, .(Count = .N), 
#                         by = .(Correlation = round(Correlation,
#                                                    1))][!is.na(Correlation)]
# plot.dt$Correlation <- factor(plot.dt$Correlation, 
...
```{r}
# plot.dt <- circ.lin.cor.cond[, .(Count = .N), 
#                              by = .(condition, 
#                                     Correlation = round(Correlation, 1))][!is.na(Correlation)]
# plot.dt$Correlation <- factor(plot.dt$Correlation, 
#                               levels = seq(from = -1, to = 1, by = .1), 
#                               ordered = T, exclude = NA)
# if(nrow(plot.dt) > 0){
...
```{r}
# ggplot(circ.lin.cor[!is.na(Correlation)], aes(x = "", y = Correlation)) + 
#     geom_boxplot() + 
...
```{r}
# ggplot(circ.lin.cor[!is.na(Correlation)], 
#        aes(x = ifelse(Correlation > 0, "Positive", "Negative"), 
#                          y = abs(Correlation))) + 
#     geom_boxplot() + 
...
```{r}
# plot.dt <- circ.lin.cor.cond[!is.na(Correlation)]
# if(nrow(plot.dt) > 0){
#     ggplot(plot.dt, 
#            aes(x = "", y = Correlation)) + 
#         geom_boxplot() + 
