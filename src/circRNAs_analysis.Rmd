...
N.B: only genes for which the circRNA has label _exonic_ or _intronic_ were considered. _intergenic\_spanning\_gene_ circRNA-overlapping genes were not considered.
```{r circrnas_per_gene, echo=FALSE, include=TRUE}
## Number of circRNAs per gene
# this commented code counts also genes for which circRNA label is intergenic_spanning_gene!
...
## subset circRNAs.per.gene to only reliable circRNAs
...
# circRNAs.per.gene.per.sample <- combined_gtf[gene_id != ".", 
#                                              .(circ_id = paste0(chr, ":", circ_start,  "-", circ_end, ":", strand),
#                                                gene_name), 
#                                              by = .(gene_id, sample_id)]
...
# n.circ.per.gene.per.sample <- merge(circRNAs.per.gene.per.sample, 
#                                     circrnas.morethan2methods.medrpm.table[, .(circ_id)], 
#                                     all = F, by = "circ_id")
...
# n.circ.per.gene.per.sample <- n.circ.per.gene.per.sample[, .(n_circRNAs = length(unique(circ_id)), 
#                                                              circ_ids = paste0(unique(circ_id), collapse = "|")), 
#                                                          by = .(gene_id, gene_name, sample_id)][order(sample_id, -n_circRNAs)]
...
if(intgroup.dt[, .N, by=condition][, .N] > 1){
    ...
    # circRNAs.per.gene.per.condition <- combined_gtf[gene_id != ".", 
    #                                                 .(circ_id = paste0(chr, ":", circ_start,  "-", circ_end, ":", strand),
    #                                                   gene_name), 
    #                                                 by = .(gene_id, condition)]
    # n.circ.per.gene.per.condition <- merge(circRNAs.per.gene.per.condition, 
    #                                        circrnas.morethan2methods.medrpm.table[, .(circ_id)], 
    #                                        all = F, by = "circ_id")
    # n.circ.per.gene.per.condition <- n.circ.per.gene.per.condition[, .(n_circRNAs = length(unique(circ_id)), 
    #                                                                    circ_ids = paste0(unique(circ_id), 
    #                                                                                      collapse = "|")), 
    #                                                                by = .(gene_id, gene_name,
    #                                                                       condition)][order(condition, -n_circRNAs)]
...
## CircRNA expression vs circRNAs expressed per gene
...
## Expression of genes overlapping the reliable circRNAs
...
## plot only expressed circRNA-overlapping genes
...
## CircRNA and host gene expression plots

In this section only the `r expressed_genes[FPKM > 0, .N, by = tracking_id][, .N]` expressed circRNA-overlapping genes are considered
...
# corr.density.plot <- ggplot(data = genic_circrnas_corr.w.labels, 
#                             mapping = aes(x = corr)) + 
#     geom_density(aes(y=..scaled..)) + geom_density(aes(y=..scaled.., color = simple_label))
