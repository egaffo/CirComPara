...
circrnas.gtf <- fread(circrnas.gtf.file, data.table = T, colClasses = colClasses)
## store fusion chromosome circRNAs in a separate variable
fchrom_prefix <- "zz"
f.circrnas.gtf <- circrnas.gtf[grepl(paste0("^",fchrom_prefix), V1)]
## filter low read count circRNAs and exclude possible fusion chromosome circRNAs
circrnas.gtf <- circrnas.gtf[V6 >= min_reads][!grepl(paste0("^",fchrom_prefix), V1)]
...
```{r corr_density_plot, fig.height=4, fig.width=8}
# plot correlations
#corr.density.plot <- ggplot(data = circrna_gene_xpr_bysample_corr, mapping = aes(x = corr)) + geom_density(aes(y=..scaled..))
corr.density.plot <- ggplot(data = genic_circrnas_corr, mapping = aes(x = corr)) + geom_density(aes(y=..scaled..))
...
#make HTML version of correlation table
```

# Fusion circRNAs

```{r f-circRNA_functions, echo=FALSE, include=FALSE}

# Check whether the backsplice spans two genes in the fusion-gene pseudo-chromosome

is.fusionCircrna <- function(chr, start, end, fusion.chr.prefix = "zz"){
    label <- "circRNA"
    if(grepl(paste0("^", fusion.chr.prefix), chr)){
        label <- "circRNA_fchr"
        chr_fields <- strsplit(chr, "_", fixed = T)[[1]]
        if(chr_fields[1] == fusion.chr.prefix){
            g1_len <- as.integer(chr_fields[4]) - as.integer(chr_fields[3])
            if(start <= g1_len && end > g1_len){
                label <- "f_circRNA"
            }
        }
    }
...
```{r f-circRNA_load, echo=FALSE}
## reload circrnas with no filters and selecting only pseudo-chromosomes
f.colClasses <- colClasses
f.exists_circrnas <- FALSE
if(nrow(f.circrnas.gtf) > 0){
    f.exists_circrnas <- TRUE
    f.circrnas.gtf[, `:=` (sample_id = sub('.*sample_id "([^"]*)".*', "\\1", V9))
                   ][, (V9newCols) := tstrsplit(sub(V9pattern, "\\1@\\2", V9), "@", fixed=TRUE)
                     ][, V9 := NULL][, f_circRNA := is.fusionCircrna(as.character(V1), V4, V5), 
                                     by = .(gene_id, V1, V4, V5)][f_circRNA != "circRNA", ]
    
    ## for canonical circRNAs in fusion chromosomes apply the standard filters
    f.circrnas.gtf[f_circRNA == "circRNA_fchr", pass_minreads := V6 >= min_reads]
    f.circrnas.gtf[pass_minreads == TRUE, pass_minmet := (.N >= min_methods), by = .(gene_id, sample_id)]
    f.circrnas.gtf <- f.circrnas.gtf[f_circRNA == "f_circRNA" | (pass_minreads & pass_minmet)][
        , `:=`(pass_minreads = NULL, pass_minmet = NULL)][]
    
}else{
    f.circrnas.gtf[, `:=`(gene_id= character())]
    f.circrna.rpms <- f.circrnas.gtf
    f.circrna.rpms.genes <- f.circrna.rpms
    f.circrna.rpms.genes[, `:=`(circ_id= character(), gene_ids= character(), gene_names= character(), 
                                label= character(), sample_id= character(), chr= character(), V1= character(), 
                                start= character(), V4= character(), end= character(), V5= character(), 
                                strand= character(), V7= character(), medRPM= character())]
}

if(f.exists_circrnas & exists_gene_overlaps){
    f.circrna.rpms <- data.table::dcast(data = f.circrnas.gtf, 
                                        formula = sample_id + gene_id + f_circRNA +V1 + V4 + V5 + V7 ~ V2, 
                                        value.var = "V6", fill = 0)
    setkey(f.circrna.rpms, gene_id)
    setkey(circ_to_genes, circ_id)
    f.circrna.rpms.genes <- circ_to_genes[f.circrna.rpms]
}

######### Elaborate f-circRNAs
fcircrnas.file <- file.path(results.dir, "fusion_circrnas.csv")
fchrom.normal.circrnas.file <- file.path(results.dir, "chimeric_chr_canonical_circrnas.csv")

if(enable_fcircrna_report){
    
    if(nrow(f.circrna.rpms.genes) > 0){
        
        fusion.circrnas <- f.circrna.rpms.genes[f_circRNA == "f_circRNA"]
        write.csv(x = fusion.circrnas, file = fcircrnas.file, row.names = F)
        
        fchrom.normal.circrnas <- f.circrna.rpms.genes[f_circRNA == "circRNA_fchr"]
        write.csv(x = fchrom.normal.circrnas, file = fchrom.normal.circrnas.file, row.names = F)
        
    }else{
        fusion.circrnas <- data.table(circ_id = character(), 
                                      f_circRNA = character(), 
                                      fchr = character(),  
                                      sample_id = character(), 
                                      N = character(),
                                      V1 = character())
        fchrom.normal.circrnas <- fusion.circrnas
    }
}

```

N.B: fusion-circRNAs are reported with no filters (either minimum reads and joint method detection).  
Detected `r fusion.circrnas[, .N, by = .(circ_id)][, .N]` fusion-circRNAs, in `r fusion.circrnas[, .N, by = .(V1)][, .N]` different fusion gene(s), from `r fusion.circrnas[, .N, by = .(sample_id)][, .N]` samples.  
Fusion-circRNAs have been saved in `r fcircrnas.file` file.  

```{r fcirc_table, echo=FALSE}
if(enable_fcircrna_report){
    fusion.circrnas.counts <- fusion.circrnas[, .(count = .N), by = .(sample_id, fchr = V1)][order(sample_id, -count)]
    knitr::kable(fusion.circrnas.counts, align = "c", caption = "Fusion circular RNAs")
}else{
    "Fusion circRNA search not enabled"
}
```

## Canonical circRNAs in chimeric chromosomes

CircRNAs detected in chimeric chromosomes = `r fchrom.normal.circrnas[, .N]`, in `r fchrom.normal.circrnas[, .N, by = .(V1)][, .N]` chimeric chromosome(s), from `r paste0(fchrom.normal.circrnas[, .N, by = .(sample_id)][, sample_id], collapse = ",")` samples.  
Fusion-circRNAs have been saved in `r fchrom.normal.circrnas.file` file.

```{r fchrom_circ_table, echo=FALSE}
if(enable_fcircrna_report){
    fchrom.circrnas.counts <- fchrom.normal.circrnas[, .(count = .N),  by = .(fchr = V1, sample_id)][order(sample_id, -count)]
    knitr::kable(fchrom.circrnas.counts, align = "c", caption = "Canonical circular RNAs in chimeric chromosomes")
}else{
    "Fusion circRNA search not enabled"
}
```
