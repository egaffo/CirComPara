...
ggplot(data = circ_met.to.plot, mapping = aes(x = factor(n_methods))) + 
    geom_bar(aes(fill = c_methods), stat = "count", position = "stack") + 
    geom_text(stat = 'count', aes(label=..count..), vjust = -0.3, 
              size = default.font.size*(5/14)) + 
    geom_text(stat = 'count', aes(label = "", y = ..count.. +1)) + 
    scale_y_continuous(expand = c(0, 0)) + 
    scale_fill_discrete("Method combinations") +
    xlab("Number of methods") + ylab("CircRNA number") +
    ggtitle(paste("Number of circRNAs (with \u2265", 
                  min_reads, "reads)\ncommonly detected by the methods")) +
    theme_bw() + 
...
#     
#     intgroup.dt <- gene_meta[, .(sample_id, condition)][order(condition, sample_id)]
...
# }else{
    ...
    intgroup.dt <- meta[, .(sample_id, condition)][order(condition, sample_id)]
    intgroup.dt[, color_luminance := 100-(10*1:.N), by = condition]
    intgroup.dt <- merge(intgroup.dt, 
                         intgroup.dt[, .(samples_per_condition = .N), 
                                     by = condition][, hue := brewer.pal(8, 
...
if(file.info(gene_fpkm_file)$size > 0){
    ...
    # }
    gene_fpkm <- melt(fread(gene_fpkm_file), id.vars = "gene", variable.name = "sample", value.name = "FPKM")
    expressed_genes <- gene_fpkm[, `:=`(FPKM = round(FPKM, digits = 8), tracking_id = gene, sample_id = sample)][]#[FPKM > 0]
