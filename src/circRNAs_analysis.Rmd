...
if(nrow(circrnas.gtf[V6 >= min_reads]) > 0){
  ...
  if(!stranded.lib){
      ## remove starnd info from circrna names and column by setting all to "+"
      # circrnas.gtf[, `:=`(V7 = "+",
      #                     gene_id = sub("(.*):[+-](.*)", "\\1:+\\2", gene_id),
      #                     transcript_id = sub("(.*):[+-](.*)", "\\1:+\\2", transcript_id))]
...
circ.lin.xpr <- merge(circrna.rpms[circrnas.morethan2methods],
      ...
      all.x = T, all.y = F)[, .(sample_id, circ_id = gene_id, 
                                circ.reads = medRPM, 
                                lin.reads, CLR = (2*medRPM)/lin.reads)]
```
