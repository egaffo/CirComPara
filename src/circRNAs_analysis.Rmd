...
if(ncol(circ_to_genes) == 1){
    ...
    ## we must merge them
    circ_to_genes <- circ_to_genes[, lapply(.SD, function(x)paste0(sort(unique(x)), 
                                                                   collapse = "|")), 
                  by = circ_id]
}
...
if(ncol(circ_to_gene) == 1){
    ...
    intergenic.circ_to_gene <- 
        circ_to_gene[gene_region == "intergenic", 
                     lapply(.SD, function(x)paste0(sort(unique(x)), 
                                                   collapse = "|")), 
                     by = .(circ_id)]
    
    circ_to_gene <- circ_to_gene[gene_region != "intergenic", 
                                 lapply(.SD, function(x)paste0(sort(unique(x)), 
                                                                   collapse = "|")), 
                  by = .(circ_id, gene_name, gene_id)]
    
    circ_to_gene <- unique(rbindlist(list(circ_to_gene,
                                          intergenic.circ_to_gene), 
                                     use.names = T))
...
circ.per.method <- circrnas.gtf[reads >= min_reads, .N, by = .(circ_id, method)]
...
ggplot(data = circ.per.method[, .(shared_by = .N), 
    ...
    ylab("CircRNAs") +
    xlab(NULL) +
    ggtitle("Proportions of commonly detected circRNAs") +
    # coord_polar(theta = "y") +
    theme_bw() +
