...
# fix_testrealign <- TRUE

# results.dir <- "/home/enrico/QKI/qki_test_data/analysis/circrnas_analysis/"
# circrnas.gtf.file <- "/home/enrico/QKI/qki_test_data/analysis/circRNA_collection/circrnas.gtf"
# combined_gtf_file <- "/home/enrico/QKI/qki_test_data/analysis/circRNA_collection/combined_circrnas.gtf.gz"
# gene_meta_file <- ""
# gene_fpkm_file  <- "/home/enrico/QKI/qki_test_data/analysis/cuffdiff/genes.fpkm_tracking"
# 

dir.create(path = results.dir, showWarnings = F, recursive = T)
knitr::opts_chunk$set(fig.width=6, fig.height=4, dev=c('svg', 'png') , 
...
sharedByMethod.barplot <- ggplot(data = circ.per.method[, .(shared_by = .N), by = gene_id], 
                                 mapping = aes(x = factor(shared_by), fill = shared_by)) + 
  geom_bar(width = .7, color = "black") + 
  geom_text(stat='count', aes(label=..count..), vjust=-0.3, size=default.font.size*(5/14)) + 
  theme_bw() + xlab("Number of methods") + ylab("CircRNA number") +
  ggtitle("Number of circRNAs, with >= 2 reads, commonly detected the methods")
## compute limits to remove space from x-axis. Add a 10% increase to the top limit
...
```{r show_circrnas_2reads_sample, echo=FALSE, include=TRUE}
...
```{r show_reliable_circrnas, echo=FALSE, include=TRUE}
circrnas.morethan2methods.medrpm[, .N, by = .(sample_id, circ_id)][, .N, by = sample_id][order(sample_id)]
```
...
circrna.xpr.per.sample.boxplot + guides(fill=FALSE) + 
  theme(text = element_text(size=default.font.size))
...
```{r show_gene_expression, echo=FALSE, include=TRUE}
sapply(X = split(expressed_genes, expressed_genes$sample_id), 
       FUN = function(x){summary(x$FPKM)})
```

```{r plot_gene_expression, echo=FALSE, include=TRUE}
...
gene.xpr.per.sample.boxplot + guides(fill=FALSE) + 
  theme(text = element_text(size=default.font.size))
...
`r ifelse(nrow(circrna_gene_xpr_bysample_corr) > 0, circrna_gene_xpr_bysample_corr[! is.na(corr)][, .N], 0)` correlations computed for `r ifelse(nrow(circrna_gene_xpr_bysample_corr) > 0, circrna_gene_xpr_bysample_corr[! is.na(corr)][, .N, by = circ_id][, .N], 0)` circRNAs and 
