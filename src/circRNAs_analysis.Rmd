...
## Expression of genes overlapping circRNAs
...
expressed_genes.by.sample <- expressed_genes[FPKM > 0, .N, by = .(sample_id, tracking_id)][, .("Tot. gene expressed" = .N), by = sample_id][order(sample_id),]
...
## Expression distribution of genes overlapping circRNAs, per each sample

```{r show_gene_expression}
kable(sapply(X = split(expressed_genes, expressed_genes$sample_id), 
       FUN = function(x){summary(x$FPKM)}), caption = "Gene expression (FPKM) distribution of genes overlapping circRNAs")

kable(sapply(X = split(expressed_genes[FPKM > 0], expressed_genes[FPKM > 0]$sample_id), 
       FUN = function(x){summary(x$FPKM)}), caption = "Gene expression (FPKM) distribution of genes overlapping circRNAs: only expressed genes")
```
...
gene.xpr.per.sample.boxplot <- ggplot(data = expressed_genes[FPKM > 0], 
  ...
  ggtitle("CircRNA-overlapping gene expression\nper sample") + theme_bw()
