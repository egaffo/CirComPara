...
## inherited variables
##
read_stats_collect.file <- "/blackhole/enrico/circular/pb/analysis/read_stats_collect/read_stats_collect.txt"
linear.mapper <- "hisat2"
circrna.reads.stats.file <- "/blackhole/enrico/circular/pb/analysis/circRNA_collection/circrna_maps_counts.txt"

results.dir <- dirname(read_stats_collect.file)
...
read_stats_collect[is.mapper == TRUE, `:=`(`Linearly unmapped` = ifelse(grepl(pattern = "aligned concordantly 0 times", x = line), 
                                                          as.integer(sub(pattern = ".* ([0-9]*) .*aligned concordantly 0 times", 
                                                              "\\1", x = line)), NA))]
...
ggplot(read.processing.table.m, aes(x=Sample, y=`Read count`)) +
