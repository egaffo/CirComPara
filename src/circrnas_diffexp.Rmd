...
non.value.columns <- c("circ_id", "strand", "gene_ids", "gene_names", 
                       "gene_biotypes", "gene_region")
...
vsl <- tryCatch(
    {vsl <- vst(dds, blind = T)}, 
    error = function(e){
        vsl <- varianceStabilizingTransformation(dds, fitType = "mean")
    }
)
...
```{r up DECs venn}
## up tab 
sign.up <- test.results.dt[significant == "yes" & log2FoldChange >= 0, ]
if(nrow(sign.up) > 0 ){
    ...
    if(n.contrasts > 1){
        ...
        if(n.contrasts < 5){fill.colors <- fill.colors +1}
        if(n.contrasts < 6){
            dec.lists.by.contrast <- split(sign.up[, circ_id],
                                           f = sign.up[, contrast])
            #venn(deg.lists.by.contrast) ## venn using gplots
            vp <- venn.diagram(dec.lists.by.contrast,
                               fill = fill.colors,
    ...
    print("No circRNA overexpressed")
}
```
...
```{r down DECs venn}
## down tab
sign.down <- test.results.dt[significant == "yes" & log2FoldChange < 0, ]
if(nrow(sign.down) > 0 ){
    ...
    if(n.contrasts > 1){
        ...
        if(n.contrasts < 5){fill.colors <- fill.colors +1}
        if(n.contrasts < 6){
            dec.lists.by.contrast <- split(sign.down[, circ_id],
